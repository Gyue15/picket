...
function displayList(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        content += `<div class="venue-item" id=${data[i].venueCode} onclick="{window.location.href='/manager/addVenue/detail?venueCode=${data[i].venueCode}'}">
...
function searchVenue() {
    ...
    $.get(`/api/venues/check-venues/${input}`).done(function (data) {
        let content = `<div class="venue-item" id=${data.orderId} onclick="{window.location.href='/manager/addVenue/detail?venueCode=${data.venueCode}'}">
